...
elseif sigma(s)~=0;
    error('given method is not explicit');
elseif rho(s)==0;
    error('something wrong with the number of steps');
...
if s>1;
    [a,b,~]=forward_euler(f,t0,y0,h,t0+s1*h,analytic);
    y(1:s1)=a(1:s1);
    for i=1:s1;
        fy(i)=f(b(i),a(i));
    end
...
for k=0:n-s;
    % calculate next value according to prescribed method
    y(s+k)= (h*(sigma(1:s1)'*fy)-(rho(1:s1)'*y(k+1:s1+k)))/rho(s); 
    fy(1:s1-1)=fy(2:s1);
    fy(s1)=f(t(s+k),y(s+k));
end
