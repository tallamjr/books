...
pixelnum = 800;     % resolution of image
...
P2derivative = zeros(length(Pderivative) - 1,1);
for it = 1:length(Pderivative)-1
    P2derivative(it)=(length(Pderivative)-it)*Pderivative(it);
end

for iteration = 1:NITER
    % each iteration considers the entire grid minus the grid pints
    % where convergence has occured
    oldi = solutions(select);

    solutions(select) = oldi - 2 .* polyval(P,oldi) .* ...
        polyval(Pderivative,oldi) ./ (2.*polyval(Pderivative,oldi).^2 ...
        - polyval(P,oldi).*polyval(P2derivative,oldi));
